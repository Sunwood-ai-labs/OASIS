...
def run_streamlit_app():
    ...
    max_retries = st.sidebar.number_input("LLMリトライ回数", min_value=1, max_value=100, value=Config.MAX_RETRIES)
    ...
    qiita_post_publish = st.sidebar.checkbox("Qiita公開設定", value=Config.QIITA_POST_PUBLISH)
    ...
    note_publish = st.sidebar.checkbox("Note公開", value=Config.NOTE_PUBLISH)
    note_api_ver = st.sidebar.text_input("Note API Ver", value=Config.NOTE_API_VER)
    zenn_output_path = st.sidebar.text_input("Zenn出力パス", value=Config.ZENN_OUTPUT_PATH)
    zenn_publish = st.sidebar.checkbox("Zenn公開", value=Config.ZENN_PUBLISH)
    firefox_binary_path = st.sidebar.text_input("Firefoxバイナリパス", value=Config.FIREFOX_BINARY_PATH or "")
    firefox_profile_path = st.sidebar.text_input("Firefoxプロファイルパス", value=Config.FIREFOX_PROFILE_PATH or "")
    firefox_headless = st.sidebar.checkbox("Firefoxヘッドレス", value=Config.FIREFOX_HEADLESS)
